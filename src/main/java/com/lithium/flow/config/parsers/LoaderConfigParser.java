 ...
 * @author Matt Ayres
 */
public class LoaderConfigParser implements ConfigParser {
	private static final Pattern pattern = Pattern.compile("!loader[ \t]+(.+?)[ \t]*");
	private final Callback callback;

	public LoaderConfigParser(Callback callback) {
	...
	public boolean parseLine(@Nonnull String line, @Nonnull ConfigBuilder builder) throws IOException {
		...
		if (matcher.matches()) {
			callback.onLoader(new FileConfigLoader(matcher.group(1)));
			return true;
		} else {
			return false;
		}
